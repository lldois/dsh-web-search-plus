...
function resolveOptions(ctx, config = {}) {
  return {
    ...
    recordRequest: (req) => {
      ctx.get?.("agents")?.currentInitiator?.()?.session?.append?.("web/deepseek-search-llm-request", req);
    },
  };
}
...
export function apply(ctx, config = {}) {
  ...
  if (ctx.inject) {
    ctx.inject(["settings"], (settingsCtx) => {
      if (Config && settingsCtx.settings?.installSection) {
...
export { Config, WebSearchPlusProvider, mapAnthropicResponse };
